...
``coord state propose/proposals/show``, ``coord plan propose/show/analyze/seams/scaffold``, ``coord plans``,
...
NAV_COORD_PLAN_ALLOWED = {"propose", "show", "analyze", "seams", "scaffold"}  # all read-only or propose-only, like show
    ...
    for seg in _SEG_SEP.split(command):
        if not _nav_segment_allowed(seg):
            return "deny", (
                ...
                "'coord plan propose/show/analyze/seams/scaffold', 'coord plans', 'coord cockpit', "
